...
int roll_no[max_size],t_b, branch_size[max_size],total_rooms,rows[min_size],cols[min_size], total_student[30];//, total_subject[30]; 
string roll[max_size],branch_name[min_size], subject_code[min_size], room_name[min_size];
fstream file; 
static int n;
...
void input()
{
	...
	for(int i=0; i<t_b; i++)
	{
		infile >>branch_name[i] >> subject_code[i];
 ...
 void remove_zero()
{
	...
	for(int i=0; i<t_b; i++)
	{
		outfile << branch_name[i] <<" " << subject_code[i] <<" "; 
